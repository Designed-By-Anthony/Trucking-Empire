<template>
  ...
  <!-- No vehicles -->
  <div v-if="fleetStore.trucks.length === 0" class="flex-1 flex items-center justify-center p-10">
  ...
  <!-- No jobs -->
  <div v-else-if="contractStore.available.length === 0" class="flex-1 flex items-center justify-center p-10">
    <div class="text-center">
      <div class="text-4xl mb-3 opacity-30">📦</div>
  ...
  <!-- Job list -->
  <div v-else class="modal-body p-4 flex flex-col gap-3">
    <div
    ...
    >
      ...
      <div class="flex items-start justify-between mb-3">
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center flex-wrap gap-1.5 mt-2">
            <span class="text-[11px] font-semibold rounded-md px-2 py-0.5" style="background: rgba(241,245,249,0.9); color: #64748b; border: 1px solid rgba(226,232,240,0.8);">
              {{ contract.distance_miles }} mi
            </span>
            <span class="text-[11px] font-semibold rounded-md px-2 py-0.5" style="background: rgba(241,245,249,0.9); color: #64748b; border: 1px solid rgba(226,232,240,0.8);">
              {{ formatWeight(contract.weight_lbs) }}
            </span>
            <span
              class="text-[11px] font-bold rounded-md px-2 py-0.5"
              :style="isUrgent(contract.delivery_deadline)
            ...
            >
              {{ isUrgent(contract.delivery_deadline) ? '⚡ ' : '' }}{{ hoursUntil(contract.delivery_deadline) }}h
            </span>
          </div>
        </div>
        ...
        <div class="text-right flex-shrink-0 ml-4">
          <p class="text-base font-black tabular-nums" style="color: #059669; letter-spacing: -0.03em;">
      ...
      <div v-if="assigningId === contract.id" class="flex flex-col gap-2 pt-1">
        <select
          ...
          class="w-full text-sm font-medium rounded-lg px-3 py-2.5 outline-none transition-colors"
          style="background: white; border: 1px solid #e2e8f0; color: #0f172a; -webkit-appearance: none;"
        >
          <option value="">Select vehicle…</option>
          <option v-for="truck in eligibleTrucks(contract.weight_lbs)" :key="truck.id" :value="truck.id">
            {{ truck.name }} · {{ truck.fuel_level.toFixed(0) }}g fuel
          </option>
        </select>
        ...
        <select
          ...
          class="w-full text-sm font-medium rounded-lg px-3 py-2.5 outline-none"
          style="background: white; border: 1px solid #e2e8f0; color: #0f172a; -webkit-appearance: none;"
        >
          <option value="">Select driver…</option>
          <option v-for="driver in availableDriversForTruck(selectedTruck)" :key="driver.id" :value="driver.id">
            {{ driver.name }} · {{ driver.hos_drive_remaining.toFixed(0) }}h HOS
          </option>
        </select>

        <div class="flex gap-2 mt-1">
          <button
            @click="confirmDispatch(contract)"
            :disabled="!selectedTruck || !selectedDriver"
            class="flex-1 text-sm font-bold text-white rounded-lg py-2.5 transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
            style="background: #2563eb;"
          >Dispatch →</button>
          <button
            @click="assigningId = null; selectedTruck = ''; selectedDriver = ''"
            class="text-sm font-semibold rounded-lg px-4 py-2.5 transition-all active:scale-95"
      ...
      <button
        v-else
        @click="assigningId = contract.id; selectedTruck = ''; selectedDriver = ''"
        class="w-full text-sm font-bold text-white rounded-lg py-2.5 mt-1 transition-all active:scale-95"
        style="background: #2563eb;"
...
const confirmDispatch = async (contract: Contract) => {
  ...
  // Ask for push permission after first dispatch — user is clearly engaged
